import { PostsService } from './posts.service';
import { CreatePostDto } from './dto/create-post.dto';
import { UpdatePostDto } from './dto/update-post.dto';
import { QueryPostsDto } from './dto/query-posts.dto';
import { SharePostDto } from './dto/share-post.dto';
import { ReportPostDto } from './dto/report-post.dto';
export declare class PostsController {
    private readonly postsService;
    constructor(postsService: PostsService);
    create(userId: string, dto: CreatePostDto): Promise<{
        comments: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            authorId: string;
            content: string;
            mentions: string[];
            isDeleted: boolean;
            deletedAt: Date | null;
            depth: number;
            isEdited: boolean;
            postId: string;
            parentId: string | null;
        }[];
        reactions: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.ReactionType;
            postId: string;
            userId: string;
        }[];
        group: {
            id: string;
            name: string;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            cover: string | null;
            type: import("@prisma/client").$Enums.GroupType;
            isPublic: boolean;
            adminId: string;
        } | null;
        course: {
            id: string;
            level: import("@prisma/client").$Enums.CourseLevel;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            title: string;
            thumbnail: string | null;
            category: string;
            isPublished: boolean;
            teacherId: string;
        } | null;
        author: {
            id: string;
            email: string;
            password: string;
            name: string;
            avatar: string | null;
            coverPhoto: string | null;
            bio: string | null;
            role: import("@prisma/client").$Enums.UserRole;
            school: string | null;
            department: string | null;
            xp: number;
            level: number;
            coins: number;
            isOnline: boolean;
            lastSeen: Date;
            isActive: boolean;
            refreshToken: string | null;
            emailVerified: boolean;
            createdAt: Date;
            updatedAt: Date;
        };
        saves: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
        }[];
        shares: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
            note: string | null;
        }[];
        reports: {
            id: string;
            createdAt: Date;
            status: string;
            postId: string;
            userId: string;
            reason: string;
            details: string | null;
        }[];
        _count: {
            author: number;
            group: number;
            course: number;
            comments: number;
            reactions: number;
            saves: number;
            shares: number;
            reports: number;
        };
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        type: import("@prisma/client").$Enums.PostType;
        courseId: string | null;
        authorId: string;
        content: string;
        status: import("@prisma/client").$Enums.PostStatus;
        visibility: import("@prisma/client").$Enums.PostVisibility;
        images: string[];
        video: string | null;
        pdfUrl: string | null;
        pdfName: string | null;
        hashtags: string[];
        mentions: string[];
        isPinned: boolean;
        isReported: boolean;
        reportCount: number;
        isDeleted: boolean;
        deletedAt: Date | null;
        shareCount: number;
        groupId: string | null;
    }>;
    findAll(userId: string, query: QueryPostsDto): Promise<{
        data: ({
            comments: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                authorId: string;
                content: string;
                mentions: string[];
                isDeleted: boolean;
                deletedAt: Date | null;
                depth: number;
                isEdited: boolean;
                postId: string;
                parentId: string | null;
            }[];
            reactions: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                type: import("@prisma/client").$Enums.ReactionType;
                postId: string;
                userId: string;
            }[];
            group: {
                id: string;
                name: string;
                createdAt: Date;
                updatedAt: Date;
                description: string | null;
                cover: string | null;
                type: import("@prisma/client").$Enums.GroupType;
                isPublic: boolean;
                adminId: string;
            } | null;
            course: {
                id: string;
                level: import("@prisma/client").$Enums.CourseLevel;
                createdAt: Date;
                updatedAt: Date;
                description: string | null;
                title: string;
                thumbnail: string | null;
                category: string;
                isPublished: boolean;
                teacherId: string;
            } | null;
            author: {
                id: string;
                email: string;
                password: string;
                name: string;
                avatar: string | null;
                coverPhoto: string | null;
                bio: string | null;
                role: import("@prisma/client").$Enums.UserRole;
                school: string | null;
                department: string | null;
                xp: number;
                level: number;
                coins: number;
                isOnline: boolean;
                lastSeen: Date;
                isActive: boolean;
                refreshToken: string | null;
                emailVerified: boolean;
                createdAt: Date;
                updatedAt: Date;
            };
            saves: {
                id: string;
                createdAt: Date;
                postId: string;
                userId: string;
            }[];
            shares: {
                id: string;
                createdAt: Date;
                postId: string;
                userId: string;
                note: string | null;
            }[];
            reports: {
                id: string;
                createdAt: Date;
                status: string;
                postId: string;
                userId: string;
                reason: string;
                details: string | null;
            }[];
            _count: {
                author: number;
                group: number;
                course: number;
                comments: number;
                reactions: number;
                saves: number;
                shares: number;
                reports: number;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.PostType;
            courseId: string | null;
            authorId: string;
            content: string;
            status: import("@prisma/client").$Enums.PostStatus;
            visibility: import("@prisma/client").$Enums.PostVisibility;
            images: string[];
            video: string | null;
            pdfUrl: string | null;
            pdfName: string | null;
            hashtags: string[];
            mentions: string[];
            isPinned: boolean;
            isReported: boolean;
            reportCount: number;
            isDeleted: boolean;
            deletedAt: Date | null;
            shareCount: number;
            groupId: string | null;
        })[];
        meta: {
            total: number;
            nextCursor: string | null;
            hasNext: boolean;
            limit: number;
        };
    }>;
    getFeed(userId: string, cursor?: string, limit?: number): Promise<{
        data: ({
            comments: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                authorId: string;
                content: string;
                mentions: string[];
                isDeleted: boolean;
                deletedAt: Date | null;
                depth: number;
                isEdited: boolean;
                postId: string;
                parentId: string | null;
            }[];
            reactions: {
                id: string;
                createdAt: Date;
                updatedAt: Date;
                type: import("@prisma/client").$Enums.ReactionType;
                postId: string;
                userId: string;
            }[];
            group: {
                id: string;
                name: string;
                createdAt: Date;
                updatedAt: Date;
                description: string | null;
                cover: string | null;
                type: import("@prisma/client").$Enums.GroupType;
                isPublic: boolean;
                adminId: string;
            } | null;
            course: {
                id: string;
                level: import("@prisma/client").$Enums.CourseLevel;
                createdAt: Date;
                updatedAt: Date;
                description: string | null;
                title: string;
                thumbnail: string | null;
                category: string;
                isPublished: boolean;
                teacherId: string;
            } | null;
            author: {
                id: string;
                email: string;
                password: string;
                name: string;
                avatar: string | null;
                coverPhoto: string | null;
                bio: string | null;
                role: import("@prisma/client").$Enums.UserRole;
                school: string | null;
                department: string | null;
                xp: number;
                level: number;
                coins: number;
                isOnline: boolean;
                lastSeen: Date;
                isActive: boolean;
                refreshToken: string | null;
                emailVerified: boolean;
                createdAt: Date;
                updatedAt: Date;
            };
            saves: {
                id: string;
                createdAt: Date;
                postId: string;
                userId: string;
            }[];
            shares: {
                id: string;
                createdAt: Date;
                postId: string;
                userId: string;
                note: string | null;
            }[];
            reports: {
                id: string;
                createdAt: Date;
                status: string;
                postId: string;
                userId: string;
                reason: string;
                details: string | null;
            }[];
            _count: {
                author: number;
                group: number;
                course: number;
                comments: number;
                reactions: number;
                saves: number;
                shares: number;
                reports: number;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.PostType;
            courseId: string | null;
            authorId: string;
            content: string;
            status: import("@prisma/client").$Enums.PostStatus;
            visibility: import("@prisma/client").$Enums.PostVisibility;
            images: string[];
            video: string | null;
            pdfUrl: string | null;
            pdfName: string | null;
            hashtags: string[];
            mentions: string[];
            isPinned: boolean;
            isReported: boolean;
            reportCount: number;
            isDeleted: boolean;
            deletedAt: Date | null;
            shareCount: number;
            groupId: string | null;
        })[];
        nextCursor: string | null;
        hasNext: boolean;
    }>;
    getTrending(userId: string, limit?: number): Promise<({
        comments: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            authorId: string;
            content: string;
            mentions: string[];
            isDeleted: boolean;
            deletedAt: Date | null;
            depth: number;
            isEdited: boolean;
            postId: string;
            parentId: string | null;
        }[];
        reactions: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.ReactionType;
            postId: string;
            userId: string;
        }[];
        group: {
            id: string;
            name: string;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            cover: string | null;
            type: import("@prisma/client").$Enums.GroupType;
            isPublic: boolean;
            adminId: string;
        } | null;
        course: {
            id: string;
            level: import("@prisma/client").$Enums.CourseLevel;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            title: string;
            thumbnail: string | null;
            category: string;
            isPublished: boolean;
            teacherId: string;
        } | null;
        author: {
            id: string;
            email: string;
            password: string;
            name: string;
            avatar: string | null;
            coverPhoto: string | null;
            bio: string | null;
            role: import("@prisma/client").$Enums.UserRole;
            school: string | null;
            department: string | null;
            xp: number;
            level: number;
            coins: number;
            isOnline: boolean;
            lastSeen: Date;
            isActive: boolean;
            refreshToken: string | null;
            emailVerified: boolean;
            createdAt: Date;
            updatedAt: Date;
        };
        saves: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
        }[];
        shares: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
            note: string | null;
        }[];
        reports: {
            id: string;
            createdAt: Date;
            status: string;
            postId: string;
            userId: string;
            reason: string;
            details: string | null;
        }[];
        _count: {
            author: number;
            group: number;
            course: number;
            comments: number;
            reactions: number;
            saves: number;
            shares: number;
            reports: number;
        };
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        type: import("@prisma/client").$Enums.PostType;
        courseId: string | null;
        authorId: string;
        content: string;
        status: import("@prisma/client").$Enums.PostStatus;
        visibility: import("@prisma/client").$Enums.PostVisibility;
        images: string[];
        video: string | null;
        pdfUrl: string | null;
        pdfName: string | null;
        hashtags: string[];
        mentions: string[];
        isPinned: boolean;
        isReported: boolean;
        reportCount: number;
        isDeleted: boolean;
        deletedAt: Date | null;
        shareCount: number;
        groupId: string | null;
    })[]>;
    findById(id: string, userId: string): Promise<{
        comments: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            authorId: string;
            content: string;
            mentions: string[];
            isDeleted: boolean;
            deletedAt: Date | null;
            depth: number;
            isEdited: boolean;
            postId: string;
            parentId: string | null;
        }[];
        reactions: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.ReactionType;
            postId: string;
            userId: string;
        }[];
        group: {
            id: string;
            name: string;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            cover: string | null;
            type: import("@prisma/client").$Enums.GroupType;
            isPublic: boolean;
            adminId: string;
        } | null;
        course: {
            id: string;
            level: import("@prisma/client").$Enums.CourseLevel;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            title: string;
            thumbnail: string | null;
            category: string;
            isPublished: boolean;
            teacherId: string;
        } | null;
        author: {
            id: string;
            email: string;
            password: string;
            name: string;
            avatar: string | null;
            coverPhoto: string | null;
            bio: string | null;
            role: import("@prisma/client").$Enums.UserRole;
            school: string | null;
            department: string | null;
            xp: number;
            level: number;
            coins: number;
            isOnline: boolean;
            lastSeen: Date;
            isActive: boolean;
            refreshToken: string | null;
            emailVerified: boolean;
            createdAt: Date;
            updatedAt: Date;
        };
        saves: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
        }[];
        shares: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
            note: string | null;
        }[];
        reports: {
            id: string;
            createdAt: Date;
            status: string;
            postId: string;
            userId: string;
            reason: string;
            details: string | null;
        }[];
        _count: {
            author: number;
            group: number;
            course: number;
            comments: number;
            reactions: number;
            saves: number;
            shares: number;
            reports: number;
        };
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        type: import("@prisma/client").$Enums.PostType;
        courseId: string | null;
        authorId: string;
        content: string;
        status: import("@prisma/client").$Enums.PostStatus;
        visibility: import("@prisma/client").$Enums.PostVisibility;
        images: string[];
        video: string | null;
        pdfUrl: string | null;
        pdfName: string | null;
        hashtags: string[];
        mentions: string[];
        isPinned: boolean;
        isReported: boolean;
        reportCount: number;
        isDeleted: boolean;
        deletedAt: Date | null;
        shareCount: number;
        groupId: string | null;
    }>;
    update(id: string, userId: string, dto: UpdatePostDto): Promise<{
        comments: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            authorId: string;
            content: string;
            mentions: string[];
            isDeleted: boolean;
            deletedAt: Date | null;
            depth: number;
            isEdited: boolean;
            postId: string;
            parentId: string | null;
        }[];
        reactions: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.ReactionType;
            postId: string;
            userId: string;
        }[];
        group: {
            id: string;
            name: string;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            cover: string | null;
            type: import("@prisma/client").$Enums.GroupType;
            isPublic: boolean;
            adminId: string;
        } | null;
        course: {
            id: string;
            level: import("@prisma/client").$Enums.CourseLevel;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            title: string;
            thumbnail: string | null;
            category: string;
            isPublished: boolean;
            teacherId: string;
        } | null;
        author: {
            id: string;
            email: string;
            password: string;
            name: string;
            avatar: string | null;
            coverPhoto: string | null;
            bio: string | null;
            role: import("@prisma/client").$Enums.UserRole;
            school: string | null;
            department: string | null;
            xp: number;
            level: number;
            coins: number;
            isOnline: boolean;
            lastSeen: Date;
            isActive: boolean;
            refreshToken: string | null;
            emailVerified: boolean;
            createdAt: Date;
            updatedAt: Date;
        };
        saves: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
        }[];
        shares: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
            note: string | null;
        }[];
        reports: {
            id: string;
            createdAt: Date;
            status: string;
            postId: string;
            userId: string;
            reason: string;
            details: string | null;
        }[];
        _count: {
            author: number;
            group: number;
            course: number;
            comments: number;
            reactions: number;
            saves: number;
            shares: number;
            reports: number;
        };
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        type: import("@prisma/client").$Enums.PostType;
        courseId: string | null;
        authorId: string;
        content: string;
        status: import("@prisma/client").$Enums.PostStatus;
        visibility: import("@prisma/client").$Enums.PostVisibility;
        images: string[];
        video: string | null;
        pdfUrl: string | null;
        pdfName: string | null;
        hashtags: string[];
        mentions: string[];
        isPinned: boolean;
        isReported: boolean;
        reportCount: number;
        isDeleted: boolean;
        deletedAt: Date | null;
        shareCount: number;
        groupId: string | null;
    }>;
    delete(id: string, userId: string, userRole: string): Promise<{
        message: string;
    }>;
    pin(id: string, userId: string): Promise<{
        comments: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            authorId: string;
            content: string;
            mentions: string[];
            isDeleted: boolean;
            deletedAt: Date | null;
            depth: number;
            isEdited: boolean;
            postId: string;
            parentId: string | null;
        }[];
        reactions: {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            type: import("@prisma/client").$Enums.ReactionType;
            postId: string;
            userId: string;
        }[];
        group: {
            id: string;
            name: string;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            cover: string | null;
            type: import("@prisma/client").$Enums.GroupType;
            isPublic: boolean;
            adminId: string;
        } | null;
        course: {
            id: string;
            level: import("@prisma/client").$Enums.CourseLevel;
            createdAt: Date;
            updatedAt: Date;
            description: string | null;
            title: string;
            thumbnail: string | null;
            category: string;
            isPublished: boolean;
            teacherId: string;
        } | null;
        author: {
            id: string;
            email: string;
            password: string;
            name: string;
            avatar: string | null;
            coverPhoto: string | null;
            bio: string | null;
            role: import("@prisma/client").$Enums.UserRole;
            school: string | null;
            department: string | null;
            xp: number;
            level: number;
            coins: number;
            isOnline: boolean;
            lastSeen: Date;
            isActive: boolean;
            refreshToken: string | null;
            emailVerified: boolean;
            createdAt: Date;
            updatedAt: Date;
        };
        saves: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
        }[];
        shares: {
            id: string;
            createdAt: Date;
            postId: string;
            userId: string;
            note: string | null;
        }[];
        reports: {
            id: string;
            createdAt: Date;
            status: string;
            postId: string;
            userId: string;
            reason: string;
            details: string | null;
        }[];
        _count: {
            author: number;
            group: number;
            course: number;
            comments: number;
            reactions: number;
            saves: number;
            shares: number;
            reports: number;
        };
    } & {
        id: string;
        createdAt: Date;
        updatedAt: Date;
        type: import("@prisma/client").$Enums.PostType;
        courseId: string | null;
        authorId: string;
        content: string;
        status: import("@prisma/client").$Enums.PostStatus;
        visibility: import("@prisma/client").$Enums.PostVisibility;
        images: string[];
        video: string | null;
        pdfUrl: string | null;
        pdfName: string | null;
        hashtags: string[];
        mentions: string[];
        isPinned: boolean;
        isReported: boolean;
        reportCount: number;
        isDeleted: boolean;
        deletedAt: Date | null;
        shareCount: number;
        groupId: string | null;
    }>;
    share(id: string, userId: string, dto: SharePostDto): Promise<{
        message: string;
    }>;
    save(id: string, userId: string): Promise<{
        saved: boolean;
    }>;
    report(id: string, userId: string, dto: ReportPostDto): Promise<{
        message: string;
    }>;
}
