...
export class MinioStorageProvider
  implements IStorageProvider, OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    this.client = new Minio.Client({
      ...
      accessKey:
        this.configService.get<string>('media.minioAccessKey') || 'minioadmin',
      secretKey:
        this.configService.get<string>('media.minioSecretKey') || 'minioadmin',
    });

    const bucket =
      this.configService.get<string>('media.minioBucket') || 'edusocial';
    const exists = await this.client.bucketExists(bucket);
    if (!exists) {
      await this.client.makeBucket(
        bucket,
        this.configService.get<string>('media.minioRegion') || 'us-east-1',
      );
  ...
  getPublicUrl(bucket: string, key: string): string {
    ...
    const useSsl =
      this.configService.get<boolean>('media.minioUseSsl') || false;
