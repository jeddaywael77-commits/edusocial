    ...
    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
      ...
      {isLoading &&
        Array.from({ length: 4 }).map((_, i) => (
          <div key={`skeleton-${i}`} className="shrink-0">
            <div className="w-24 h-40 rounded-2xl bg-muted animate-pulse" />
          </div>
        ))}

      {!isLoading && !isError && stories && stories.slice(0, 6).map((story, i) => (
        <motion.div
          key={story.id}
          initial={{ opacity: 0, scale: 0.9 }}
          animate={{ opacity: 1, scale: 1 }}
